...
#' srDev  \code{FLQuant} recuitment deviates on the log scale, i.e. multiplicative
...
setMethod('hcrICES', signature(object="FLStock",eql='FLBRP'),
  ...
  for (iYr in seq(start,end-1,interval)){
    ...
    ##TAC Bounds
    flag=c(ssb(mp)[,ac(stkYrs)]<c(FLPar(params)["blim"])[1])
    rtn=res[[1]]
    ...
    object=fwd(object,catch=rtn,sr=eql,residuals=srDev)
