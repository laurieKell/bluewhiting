...
dirDat="/home/laurence/Desktop/Dropbox/PELAC/bluewhiting/data"
...
```{r, om2}
load(file.path(dirDir,"samData.RData"))
...
save(retro, file=file.path(dirDat,"om.RData"),compress="xz")
```
```{r, retro}
load(file.path(dirSAM,"modelData.RData"))
...
```{r, ctcn, fig.height=4, fig.width=8}
...
```{r, sg, fig.height=5, fig.width=6}
sg=fmle(as.FLSR(om1,model="segreg"),control=list(silent=TRUE))

ggplot(model.frame(FLQuants(rec    =fitted(sg), 
                            ssb    =ssb(sg),
...
```{r, bhsv, fig.height=5, fig.width=6}
sr9=fmle(as.FLSR(om1,model="bevholtSV"),
                fixed=c(s=0.9,spr0=spr0(eql)),
                        control=list(silent=TRUE))

ggplot(model.frame(FLQuants(rec    =fitted(sr9), 
                            ssb    =ssb(sr9),
...
```{r, sg-eql, fig.height=6, fig.width=6}
...
```{r, bh-eql9, fig.height=6, fig.width=8}
...
dat=mdply(as.numeric(dimnames(m(om1))$year[-seq(3)]),function(year){
      x =window(om1,start=year-3,end=year)
...
dat$quant=factor(dat$quant,levels=c("rec","ssb","yield","harvest"),
                           labels=c("Recruite","SSB","Yield","F"))  
ggplot(dat)+
  geom_line(aes(as.numeric(X1),data,col=refpt))+
...
library(grid)
source('/home/laurence/Desktop/Dropbox/PELAC/bluewhiting/R/kobe-phaseMar.R')
source('/home/laurence/Desktop/Dropbox/PELAC/bluewhiting/R/kobe-phase.R')
