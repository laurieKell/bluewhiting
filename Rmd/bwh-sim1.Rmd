...
dirMy="/home/laurence/Desktop/Dropbox/PELAC/bluewhiting"
dirDat=file.path(dirMy,"data")
```
```{r}
source(file.path(dirMy,"R/lFig.R"))
iFig=25
```
```{r}
refpts=FLPar(c(
...
```{r, smry}
load(file.path(dirDat,"data/sims.RData"))   
```
...
```{r, fig.height=8,fig.width=8}
plot(FLStocks("HCR I" =sims[["acf1"]][[1]],
              "HCR II"=sims[["acf2"]][[1]]))+ 
  theme_bw()+
  theme(legend.position="bottom")+
  scale_colour_manual("Management",values=rainbow(2))+
...
qtl=cbind(Scenario=rep(names(sims[-(1:2)]),each=dim(qtl)[1]/10),qtl)
...
its=cbind(Scenario=rep(names(sims[-(1:2)]),each=dim(its)[1]/10),its)
